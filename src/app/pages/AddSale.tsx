...
import { Plus, Clock, X, Loader2, CheckCircle2, AlertCircle, Search, Package } from "lucide-react";
...
/** A product entry derived from real sale history or manually typed */
...
export function AddSale() {
  ...
  // ── Load product catalog from past sales ─────────────────────────────────
  useEffect(() => {
    const loadProducts = async () => {
      setIsLoadingProducts(true);
      try {
        const raw = await apiFetch<unknown>("/sales");
        let salesList: unknown[] = [];
        if (Array.isArray(raw)) {
          salesList = raw;
        } else if (raw && typeof raw === "object") {
          const obj = raw as Record<string, unknown>;
          const candidate = obj.data ?? obj.items ?? obj.sales ?? obj.results;
          if (Array.isArray(candidate)) salesList = candidate;
        }

        // Extract unique products from all sale items
        ...
        for (const sale of salesList) {
          ...
          for (const item of items) {
            ...
            if (sku && name && !seen.has(sku)) {
              seen.set(sku, { sku, name, unitCost });
            }
          }
        }
        setProductCatalog(Array.from(seen.values()));
      } catch {
        // Silently fall back — user can still manually enter products
      } finally {
  ...
  useEffect(() => {
    if (showProductPicker && !manualEntry) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setSubmitError(err instanceof Error ? err.message : "Failed to submit sale");
    } finally {
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-5">
          ...
          {submitSuccess && (
            <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg text-green-700 text-sm flex items-center gap-2">
              ...
              Sale submitted successfully! Redirecting…
            </div>
          )}
          ...
          {/* ── Row 1: Date / Biller / Paid By / Discount ── */}
